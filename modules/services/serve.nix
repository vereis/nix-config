...
with lib;
{
      ...
      type = types.attrsOf (
        types.submodule {
          options = {
            ...
            ssl = mkOption { type = types.bool; default = false; };
            ...
            ddns = mkOption {
              type = types.nullOr (types.submodule {
                options = {
                  ...
                  zone = mkOption { type = types.nullOr types.str; default = null; };
                };
              });
              default = null;
  ...
  config = mkIf config.modules.serve.enable {
    ...
    environment.etc."ddclient.conf" = let
      enabledDomains = lib.filterAttrs (name: proxy: proxy.ddns != null && proxy.ddns.enable) config.modules.serve.sites;
      hasEnabledDomains = enabledDomains != {};
    in mkIf (hasEnabledDomains) {
      text = ''
        use=web
        web=ipinfo.io/ip
        ssl=yes
        ...
        builtins.mapAttrs
          (host: siteConfig:
            let
              ...
              finalExtraConfig = if generatedConfig != "" then
