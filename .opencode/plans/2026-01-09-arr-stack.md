...
Extend the existing `media-server.nix` module to include the full *arr media acquisition stack: **Sonarr (shows)**, **Sonarr (anime, separate instance)**, **Radarr**, **Prowlarr**, **Lidarr**, **Bazarr**, **Jellyseerr**, and **qBittorrent**.
...
- Add Sonarr (shows), Sonarr (anime), Radarr, Prowlarr, Lidarr, Bazarr, Jellyseerr, qBittorrent to kyubey
...
- Sonarr anime instance must be **Sonarr v4** compatible (required for anime CF/scoring from TRaSH)
...
- For shows: use the default instance, and set Series Type appropriately per series.
- For anime: use the separate instance and follow TRaSH Sonarr anime quality profile/scoring.

4. **Configure qBittorrent** (via web UI)

- Verify download path is `/storage/torrents`
- Set Torrent Management Mode to `Automatic`
- Create categories: `movies`, `shows`, `anime`, `music`

5. **Configure Prowlarr**

- Add indexers
- Connect to Sonarr (both instances), Radarr, Lidarr
